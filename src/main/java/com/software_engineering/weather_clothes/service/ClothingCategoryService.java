...
public class ClothingCategoryService {
    ...
    public Map<String, List<ClothingCategory>> getClothingCategory(Weather weather) {
        ...
        int t1h = weather.getT1h();
        TemperatureRange temperatureRange = TemperatureRange.getRange(t1h);

        switch (temperatureRange) {
            case VERY_HOT:
                addVeryHotClothing(recommendations);
                break;
            case HOT:
                addHotClothing(recommendations);
                break;
            case WARM:
                addWarmClothing(recommendations);
                break;
            case MILD:
                addMildClothing(recommendations);
                break;
            case COOL:
                addCoolClothing(recommendations);
                break;
            case COLD:
                addColdClothing(recommendations);
                break;
            case VERY_COLD:
                addVeryColdClothing(recommendations);
                break;
        }
    ...
    private void addVeryHotClothing(Map<String, List<ClothingCategory>> recommendations){
        // 상의
...
//        addClothingCategory(recommendations, "원피스/스커트", "100006"); // 롱스커트
    ...
    private void addMildClothing(Map<String, List<ClothingCategory>> recommendations){
        // 아우터
    ...
    private void addCoolClothing(Map<String, List<ClothingCategory>> recommendations){
        // 아우터
        addClothingCategory(recommendations, "아우터", "002002"); // 라이더스 재킷
        addClothingCategory(recommendations, "아우터", "002004"); // 스타디움 재킷
        addClothingCategory(recommendations, "아우터", "002017"); // 트러커 재킷
    ...
    private void addColdClothing(Map<String, List<ClothingCategory>> recommendations){
        // 아우터
    ...
    private void addVeryColdClothing(Map<String, List<ClothingCategory>> recommendations){
        // 아우터
        addClothingCategory(recommendations, "아우터", "002023"); // 플리스/뽀글이
        addClothingCategory(recommendations, "아우터", "002007"); // 겨울 싱글 코트
        addClothingCategory(recommendations, "아우터", "002024"); // 겨울 더블 코트
    ...
    public Map<String, Map<String, List<ClothingProduct>>> getClothingProductsFromCategories(
        ...
        for (Map.Entry<String, List<ClothingCategory>> entry : clothingCategories.entrySet()) {
            ...
            for (ClothingCategory category : categories) {
                // 해당 카테고리의 상품들을 likes 내림차순으로 최대 12개 조회
